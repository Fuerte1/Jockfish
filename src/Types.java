
public class Types {

//#ifndef TYPES_H_INCLUDED
//    #define TYPES_H_INCLUDED

// When compiling with provided Makefile (e.g. for Linux and OSX), configuration
// is done automatically. To get started type 'make help'.
//
// When Makefile is not used (e.g. with Microsoft Visual Studio) some switches
// need to be set manually:
//
// -DNDEBUG      | Disable debugging mode. Always use this for release.
//
// -DNO_PREFETCH | Disable use of prefetch asm-instruction. You may need this to
//               | run on some very old machines.
//
// -DUSE_POPCNT  | Add runtime support for use of popcnt asm-instruction. Works
//               | only in 64-bit mode and requires hardware with popcnt support.
//
// -DUSE_PEXT    | Add runtime support for use of pext asm-instruction. Works
//               | only in 64-bit mode and requires hardware with pext support.

//    #include <cassert>
//    #include <cstdint>
//
//    #if defined(_MSC_VER)
//    // Disable some silly and noisy warnings from MSVC compiler
//        #pragma warning(disable: 4127)  // Conditional expression is constant
//        #pragma warning(disable: 4146)  // Unary minus operator applied to unsigned type
//        #pragma warning(disable: 4800)  // Forcing value to bool 'true' or 'false'
//    #endif

// Predefined macros hell:
//
// __GNUC__                Compiler is GCC, Clang or ICX
// __clang__               Compiler is Clang or ICX
// __INTEL_LLVM_COMPILER   Compiler is ICX
// _MSC_VER                Compiler is MSVC
// _WIN32                  Building on Windows (any)
// _WIN64                  Building on Windows 64 bit

//    #if defined(__GNUC__) && (__GNUC__ < 9 || (__GNUC__ == 9 && __GNUC_MINOR__ <= 2)) \
//            && defined(_WIN32) && !defined(__clang__)
//        #define ALIGNAS_ON_STACK_VARIABLES_BROKEN
//    #endif

//    #define ASSERT_ALIGNED(ptr, alignment) assert(reinterpret_cast<uintptr_t>(ptr) % alignment == 0)
//
//            #if defined(_WIN64) && defined(_MSC_VER)  // No Makefile used
//        #include <intrin.h>                   // Microsoft header for _BitScanForward64()
//        #define IS_64BIT
//    #endif

    public static final boolean IS_64BIT = true;

//    #if defined(USE_POPCNT) && defined(_MSC_VER)
//        #include <nmmintrin.h>  // Microsoft header for _mm_popcnt_u64()
//    #endif

    public static final boolean USE_POPCNT = false;

//    #if !defined(NO_PREFETCH) && defined(_MSC_VER)
//        #include <xmmintrin.h>  // Microsoft header for _mm_prefetch()
//    #endif

    public static final boolean NO_PREFETCH = true;

//    #if defined(USE_PEXT)
//        #include <immintrin.h>  // Header for _pext_u64() intrinsic
//        #define pext(b, m) _pext_u64(b, m)
//            #else
//            #define pext(b, m) 0
//            #endif

    public static final boolean USE_PEXT = false;

    public static int pext(Object b, Object m) {
        return 0;
    }

//    namespace Stockfish {

//    #ifdef USE_POPCNT
//        constexpr bool HasPopCnt = true;
//    #else
//        constexpr bool HasPopCnt = false;
//    #endif

    public static final boolean HasPopCnt = USE_POPCNT;

//    #ifdef USE_PEXT
//        constexpr bool HasPext = true;
//    #else
//        constexpr bool HasPext = false;
//    #endif

    public static final boolean HasPext = USE_PEXT;

//    #ifdef IS_64BIT
//        constexpr bool Is64Bit = true;
//    #else
//        constexpr bool Is64Bit = false;
//    #endif

    public static final boolean Is64Bit = IS_64BIT;

//        using Key      = uint64_t;
//        using Bitboard = uint64_t;

    public static class Key {
        public long value; // 64 bits
    }

    public static class Bitboard {
        public long value; // 64 bits
    }

//        constexpr int MAX_MOVES = 256;
//        constexpr int MAX_PLY   = 246;

    public static final int MAX_MOVES = 256;
    public static final int MAX_PLY = 246;

//        enum Color {
//            WHITE,
//            BLACK,
//            COLOR_NB = 2
//        };

    public enum Color {
        WHITE, // ordinal: 0
        BLACK; // ordinal: 1

        private static final Color[] values = values();

        public Color toggle() {
            return values[this.ordinal() ^ 1];
        }

        public static Color of(Piece pc) {
            return values[pc.value >>> 3];
        }

        private static final Direction[] pawnDirection = new Direction[]{Direction.NORTH, Direction.SOUTH};

        public Direction pawn_push() {
            return pawnDirection[this.ordinal()];
        }

    }

    public static final int COLOR_NB = 2;

//        enum CastlingRights {
//            NO_CASTLING,
//            WHITE_OO,
//            WHITE_OOO = WHITE_OO << 1,
//            BLACK_OO  = WHITE_OO << 2,
//            BLACK_OOO = WHITE_OO << 3,
//
//            KING_SIDE      = WHITE_OO | BLACK_OO,
//            QUEEN_SIDE     = WHITE_OOO | BLACK_OOO,
//            WHITE_CASTLING = WHITE_OO | WHITE_OOO,
//            BLACK_CASTLING = BLACK_OO | BLACK_OOO,
//            ANY_CASTLING   = WHITE_CASTLING | BLACK_CASTLING,
//
//            CASTLING_RIGHT_NB = 16
//        };

    public enum CastlingRights {
        NO_CASTLING(0),
        WHITE_OO(1),
        WHITE_OOO(2), // = WHITE_OO << 1,
        BLACK_OO(4), // = WHITE_OO << 2,
        BLACK_OOO(8), // = WHITE_OO << 3,

        KING_SIDE(1 | 4), // = WHITE_OO | BLACK_OO,
        QUEEN_SIDE(2 | 8), // = WHITE_OOO | BLACK_OOO,
        WHITE_CASTLING(1 | 2), // = WHITE_OO | WHITE_OOO,
        BLACK_CASTLING(4 | 8), // = BLACK_OO | BLACK_OOO,
        ANY_CASTLING(1 | 2 | 4 | 8), // = WHITE_CASTLING | BLACK_CASTLING,

        CASTLING_RIGHT_NB(16);

        public final int value;

        CastlingRights(int value) {
            this.value = value;
        }

    }

//    enum Bound {
//            BOUND_NONE,
//            BOUND_UPPER,
//            BOUND_LOWER,
//            BOUND_EXACT = BOUND_UPPER | BOUND_LOWER
//        };

    public enum Bound {
        BOUND_NONE, // 0
        BOUND_UPPER, // 1
        BOUND_LOWER, // 2
        BOUND_EXACT // 3 =BOUND_UPPER |BOUND_LOWER
    }

    // Value is used as an alias for int, this is done to differentiate between a search
// value and any other integer value. The values used in search are always supposed
// to be in the range (-VALUE_NONE, VALUE_NONE] and should not exceed this range.
//        using Value = int;

    public static class Value {
        public int value;
    }

//        constexpr Value VALUE_ZERO     = 0;
//        constexpr Value VALUE_DRAW     = 0;
//        constexpr Value VALUE_NONE     = 32002;
//        constexpr Value VALUE_INFINITE = 32001;

    public static final int VALUE_ZERO     = 0;
    public static final int VALUE_DRAW     = 0;
    public static final int VALUE_NONE     = 32002;
    public static final int VALUE_INFINITE = 32001;

//        constexpr Value VALUE_MATE             = 32000;
//        constexpr Value VALUE_MATE_IN_MAX_PLY  = VALUE_MATE - MAX_PLY;
//        constexpr Value VALUE_MATED_IN_MAX_PLY = -VALUE_MATE_IN_MAX_PLY;

    public static final int VALUE_MATE             = 32000;
    public static final int VALUE_MATE_IN_MAX_PLY  = VALUE_MATE - MAX_PLY;
    public static final int VALUE_MATED_IN_MAX_PLY = -VALUE_MATE_IN_MAX_PLY;

//    constexpr Value VALUE_TB                 = VALUE_MATE_IN_MAX_PLY - 1;
//        constexpr Value VALUE_TB_WIN_IN_MAX_PLY  = VALUE_TB - MAX_PLY;
//        constexpr Value VALUE_TB_LOSS_IN_MAX_PLY = -VALUE_TB_WIN_IN_MAX_PLY;

    public static final int VALUE_TB                 = VALUE_MATE_IN_MAX_PLY - 1;
    public static final int VALUE_TB_WIN_IN_MAX_PLY  = VALUE_TB - MAX_PLY;
    public static final int VALUE_TB_LOSS_IN_MAX_PLY = -VALUE_TB_WIN_IN_MAX_PLY;

//        constexpr bool is_valid(Value value) { return value != VALUE_NONE; }

    public static boolean is_valid(int value) { return value != VALUE_NONE; }

//    constexpr bool is_win(Value value) {
//            assert(is_valid(value));
//            return value >= VALUE_TB_WIN_IN_MAX_PLY;
//        }

    public static boolean is_win(int value) {
        assert(is_valid(value));
        return value >= VALUE_TB_WIN_IN_MAX_PLY;
    }

//    constexpr bool is_loss(Value value) {
//            assert(is_valid(value));
//            return value <= VALUE_TB_LOSS_IN_MAX_PLY;
//        }

    public static boolean is_loss(int value) {
        assert(is_valid(value));
        return value <= VALUE_TB_LOSS_IN_MAX_PLY;
    }

//    constexpr bool is_decisive(Value value) { return is_win(value) || is_loss(value); }

    public static boolean is_decisive(int value) { return is_win(value) || is_loss(value); }

    // In the code, we make the assumption that these values
// are such that non_pawn_material() can be used to uniquely
// identify the material on the board.
//        constexpr Value PawnValue   = 208;
//        constexpr Value KnightValue = 781;
//        constexpr Value BishopValue = 825;
//        constexpr Value RookValue   = 1276;
//        constexpr Value QueenValue  = 2538;

    public static final int PawnValue   = 208;
    public static final int KnightValue = 781;
    public static final int BishopValue = 825;
    public static final int RookValue   = 1276;
    public static final int QueenValue  = 2538;


    // clang-format off
//        enum PieceType {
//            NO_PIECE_TYPE, PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING,
//            ALL_PIECES = 0,
//            PIECE_TYPE_NB = 8
//        };

    public enum PieceType {
        NO_PIECE_TYPE,
        PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING,
        ;

        private static final PieceType[] values = values();

        public static PieceType of(Piece pc) {
            return values[pc.value & 7];
        }

        public static PieceType of(int i) {
            return values[i];
        }

    }

    public static final int ALL_PIECES = 0;
    public static final int PIECE_TYPE_NB = 8;

//        enum Piece {
//            NO_PIECE,
//            W_PAWN = PAWN,     W_KNIGHT, W_BISHOP, W_ROOK, W_QUEEN, W_KING,
//            B_PAWN = PAWN + 8, B_KNIGHT, B_BISHOP, B_ROOK, B_QUEEN, B_KING,
//            PIECE_NB = 16
//        };

    public enum Piece {
        NO_PIECE(0),
        W_PAWN(1), W_KNIGHT(2), W_BISHOP(3), W_ROOK(4), W_QUEEN(5), W_KING(6),
        B_PAWN(1 + 8), B_KNIGHT(10), B_BISHOP(11), B_ROOK(12), B_QUEEN(13), B_KING(14),
        PIECE_NB(16);

        public final int value;

        Piece(int value) {
            this.value = value;
        }

        private static final Piece[] valueList = new Piece[]{
                NO_PIECE, // 0
                W_PAWN, W_KNIGHT, W_BISHOP, W_ROOK, W_QUEEN, W_KING,
                NO_PIECE, // 7
                NO_PIECE, // 8
                B_PAWN, B_KNIGHT, B_BISHOP, B_ROOK, B_QUEEN, B_KING,
        };

        public static Piece of(Color c, PieceType pt) {
            return valueList[(c.ordinal() << 3) + pt.ordinal()];
        }

        public Piece toggle() {
            return valueList[this.value ^ 8];
        }

    }

    public static final int PIECE_NB = 16;

    // clang-format on

//        constexpr Value PieceValue[PIECE_NB] = {
//                VALUE_ZERO, PawnValue, KnightValue, BishopValue, RookValue, QueenValue, VALUE_ZERO, VALUE_ZERO,
//                VALUE_ZERO, PawnValue, KnightValue, BishopValue, RookValue, QueenValue, VALUE_ZERO, VALUE_ZERO};

    public static final int[] PieceValue = new int[]{ // PIECE_NB
            VALUE_ZERO, PawnValue, KnightValue, BishopValue, RookValue, QueenValue, VALUE_ZERO, VALUE_ZERO,
            VALUE_ZERO, PawnValue, KnightValue, BishopValue, RookValue, QueenValue, VALUE_ZERO, VALUE_ZERO};

//        using Depth = int;

    public static class Depth {
        public int value;
    }

//        enum : int {
//            // The following DEPTH_ constants are used for transposition table entries
//            // and quiescence search move generation stages. In regular search, the
//            // depth stored in the transposition table is literal: the search depth
//            // (effort) used to make the corresponding transposition table value. In
//            // quiescence search, however, the transposition table entries only store
//            // the current quiescence move generation stage (which should thus compare
//            // lower than any regular search depth).
//            DEPTH_QS = 0,
//                    // For transposition table entries where no searching at all was done
//                    // (whether regular or qsearch) we use DEPTH_UNSEARCHED, which should thus
//                    // compare lower than any quiescence or regular depth. DEPTH_ENTRY_OFFSET
//                    // is used only for the transposition table entry occupancy check (see tt.cpp),
//                    // and should thus be lower than DEPTH_UNSEARCHED.
//                    DEPTH_UNSEARCHED   = -2,
//                    DEPTH_ENTRY_OFFSET = -3
//        };

    public static final int DEPTH_QS = 0;
    public static final int DEPTH_UNSEARCHED   = -2;
    public static final int DEPTH_ENTRY_OFFSET = -3;

    // clang-format off
//        enum Square : int {
//            SQ_A1, SQ_B1, SQ_C1, SQ_D1, SQ_E1, SQ_F1, SQ_G1, SQ_H1,
//                    SQ_A2, SQ_B2, SQ_C2, SQ_D2, SQ_E2, SQ_F2, SQ_G2, SQ_H2,
//                    SQ_A3, SQ_B3, SQ_C3, SQ_D3, SQ_E3, SQ_F3, SQ_G3, SQ_H3,
//                    SQ_A4, SQ_B4, SQ_C4, SQ_D4, SQ_E4, SQ_F4, SQ_G4, SQ_H4,
//                    SQ_A5, SQ_B5, SQ_C5, SQ_D5, SQ_E5, SQ_F5, SQ_G5, SQ_H5,
//                    SQ_A6, SQ_B6, SQ_C6, SQ_D6, SQ_E6, SQ_F6, SQ_G6, SQ_H6,
//                    SQ_A7, SQ_B7, SQ_C7, SQ_D7, SQ_E7, SQ_F7, SQ_G7, SQ_H7,
//                    SQ_A8, SQ_B8, SQ_C8, SQ_D8, SQ_E8, SQ_F8, SQ_G8, SQ_H8,
//                    SQ_NONE,
//
//                    SQUARE_ZERO = 0,
//                    SQUARE_NB   = 64
//        };
// clang-format on

    public enum Square {
        SQ_A1, SQ_B1, SQ_C1, SQ_D1, SQ_E1, SQ_F1, SQ_G1, SQ_H1,
        SQ_A2, SQ_B2, SQ_C2, SQ_D2, SQ_E2, SQ_F2, SQ_G2, SQ_H2,
        SQ_A3, SQ_B3, SQ_C3, SQ_D3, SQ_E3, SQ_F3, SQ_G3, SQ_H3,
        SQ_A4, SQ_B4, SQ_C4, SQ_D4, SQ_E4, SQ_F4, SQ_G4, SQ_H4,
        SQ_A5, SQ_B5, SQ_C5, SQ_D5, SQ_E5, SQ_F5, SQ_G5, SQ_H5,
        SQ_A6, SQ_B6, SQ_C6, SQ_D6, SQ_E6, SQ_F6, SQ_G6, SQ_H6,
        SQ_A7, SQ_B7, SQ_C7, SQ_D7, SQ_E7, SQ_F7, SQ_G7, SQ_H7,
        SQ_A8, SQ_B8, SQ_C8, SQ_D8, SQ_E8, SQ_F8, SQ_G8, SQ_H8,
        SQ_NONE;

        private static final Square[] values = values();

        public static Square of(int i) {
            return values[i];
        }

        public static Square of(File f, Rank r) {
            return Square.of((r.ordinal() << 3) + f.ordinal());
        }

        public static Square of(int f, int r) {
            return Square.of((r << 3) + f);
        }

        public static boolean is_ok(int s) { return s >= Square.SQ_A1.ordinal() && s <= Square.SQ_H8.ordinal(); }

        public static Square relative(Color c, Square s) {
            return values[s.ordinal() ^ (c.ordinal() * 56)];
        }

        public Square flip_rank() {
            return of(this.ordinal() ^ Square.SQ_A8.ordinal());
        }

        public Square flip_file() {
            return of(this.ordinal() ^ Types.Square.SQ_H1.ordinal());
        }

        public Square plus(Direction d) {
            return of(this.ordinal() + d.value);
        }

        public Square minus(Direction d) {
            return of(this.ordinal() - d.value);
        }

    }

    public static final int SQUARE_ZERO = 0;
    public static final int SQUARE_NB = 64;

//        enum <Direction> : int {
//            NORTH = 8,
//                    EAST  = 1,
//                    SOUTH = -NORTH,
//                    WEST  = -EAST,
//
//                    NORTH_EAST = NORTH + EAST,
//                    SOUTH_EAST = SOUTH + EAST,
//                    SOUTH_WEST = SOUTH + WEST,
//                    NORTH_WEST = NORTH + WEST
//        };

    public enum Direction {
        NORTH(8),
        EAST(1),
        SOUTH(-8),
        WEST(-1),

        NORTH_EAST(8 + 1),
        SOUTH_EAST(-8 + 1),
        SOUTH_WEST(-8 - 1),
        NORTH_WEST(8 - 1);

        public final int value;

        Direction(int value) {
            this.value = value;
        }
    }

//        enum File : int {
//            FILE_A,
//                    FILE_B,
//                    FILE_C,
//                    FILE_D,
//                    FILE_E,
//                    FILE_F,
//                    FILE_G,
//                    FILE_H,
//                    FILE_NB
//        };

    public enum File {
        FILE_A,
        FILE_B,
        FILE_C,
        FILE_D,
        FILE_E,
        FILE_F,
        FILE_G,
        FILE_H,
        FILE_NB;

        private static final File[] values = values();

        public static File of(Square square) {
            return values[square.ordinal() & 7];
        }

    }

//    enum Rank : int {
//            RANK_1,
//                    RANK_2,
//                    RANK_3,
//                    RANK_4,
//                    RANK_5,
//                    RANK_6,
//                    RANK_7,
//                    RANK_8,
//                    RANK_NB
//        };

    public enum Rank {
        RANK_1,
        RANK_2,
        RANK_3,
        RANK_4,
        RANK_5,
        RANK_6,
        RANK_7,
        RANK_8;

        private static final Rank[] values = values();

        public static Rank of(Square s) {
            return values[s.ordinal() >>> 3];
        }

        public static Rank relative(Color c, Rank r) {
            return values[r.ordinal() ^ (c.ordinal() * 7)];
        }

        public static Rank relative(Color c, Square s) {
            return relative(c, of(s));
        }

    }

    public static int RANK_NB = 8;

    // Keep track of what a move changes on the board (used by NNUE)
//        struct DirtyPiece {
//
//            // Number of changed pieces
//            int dirty_num;
//
//            // Max 3 pieces can change in one move. A promotion with capture moves
//            // both the pawn and the captured piece to SQ_NONE and the piece promoted
//            // to from SQ_NONE to the capture square.
//            Piece piece[3];
//
//            // From and to squares, which may be SQ_NONE
//            Square from[3];
//            Square to[3];
//        };

    public static class DirtyPiece {

        // Number of changed pieces
        public int dirty_num;

        // Max 3 pieces can change in one move. A promotion with capture moves
        // both the pawn and the captured piece to SQ_NONE and the piece promoted
        // to from SQ_NONE to the capture square.
        Piece[] piece = new Piece[3];

        // From and to squares, which may be SQ_NONE
        Square[] from = new Square[3];
        Square[] to = new Square[3];
    };

//    #define ENABLE_INCR_OPERATORS_ON(T) \
//        inline T& operator++(T& d) { return d = T(int(d) + 1); } \
//        inline T& operator--(T& d) { return d = T(int(d) - 1); }
//
//        ENABLE_INCR_OPERATORS_ON(PieceType)
//        ENABLE_INCR_OPERATORS_ON(Square)
//        ENABLE_INCR_OPERATORS_ON(File)
//        ENABLE_INCR_OPERATORS_ON(Rank)
//
//    #undef ENABLE_INCR_OPERATORS_ON

//        constexpr Direction operator+(Direction d1, Direction d2) { return Direction(int(d1) + int(d2)); }
//        constexpr Direction operator*(int i, Direction d) { return Direction(i * int(d)); }

    public static int DirectionPlus(Direction d1, Direction d2) {
        return d1.value + d2.value;
    }

    public static int DirectionMultiply(int i, Direction d) {
        return i * d.value;
    }

// Additional operators to add a Direction to a Square
//        constexpr Square operator+(Square s, Direction d) { return Square(int(s) + int(d)); }
//        constexpr Square operator-(Square s, Direction d) { return Square(int(s) - int(d)); }
//        inline Square&   operator+=(Square& s, Direction d) { return s = s + d; }
//        inline Square&   operator-=(Square& s, Direction d) { return s = s - d; }

    public static int SquarePlus(Square s, Direction d) {
        return s.plus(d).ordinal();
    }

    public static int SquareMinus(Square s, Direction d) {
        return s.minus(d).ordinal();
    }

// Toggle color
//        constexpr Color operator~(Color c) { return Color(c ^ BLACK); }

//    public static Color ColorToggle(Color c) { return c == Color.BLACK ? Color.WHITE : Color.BLACK; }

// Swap A1 <-> A8
//        constexpr Square flip_rank(Square s) { return Square(s ^ SQ_A8); }

// Swap A1 <-> H1
//        constexpr Square flip_file(Square s) { return Square(s ^ SQ_H1); }

// Swap color of piece B_KNIGHT <-> W_KNIGHT
//        constexpr Piece operator~(Piece pc) { return Piece(pc ^ 8); }

//        constexpr CastlingRights operator&(Color c, CastlingRights cr) {
//            return CastlingRights((c == WHITE ? WHITE_CASTLING : BLACK_CASTLING) & cr);
//        }

    public static int CastlingRightsCheck(Color c, CastlingRights cr) {
        return (c == Color.WHITE ?
                CastlingRights.WHITE_CASTLING.value :
                CastlingRights.BLACK_CASTLING.value) & cr.value;
    }

//    constexpr Value mate_in(int ply) { return VALUE_MATE - ply; }

    public static int mate_in(int ply) { return VALUE_MATE - ply; }

//        constexpr Value mated_in(int ply) { return -VALUE_MATE + ply; }

    public static int mated_in(int ply) { return -VALUE_MATE + ply; }

//        constexpr Square make_square(File f, Rank r) { return Square((r << 3) + f); }

//        constexpr Piece make_piece(Color c, PieceType pt) { return Piece((c << 3) + pt); }

//        constexpr PieceType type_of(Piece pc) { return PieceType(pc & 7); }

//        inline Color color_of(Piece pc) {
//            assert(pc != NO_PIECE);
//            return Color(pc >> 3);
//        }

//    public static Color color_of(Piece pc) {
//        assert(pc != Piece.NO_PIECE);
//        return Color.of(pc);
//    }

//        constexpr bool is_ok(Square s) { return s >= SQ_A1 && s <= SQ_H8; }

//        constexpr File file_of(Square s) { return File(s & 7); }

//        constexpr Rank rank_of(Square s) { return Rank(s >> 3); }

//        constexpr Square relative_square(Color c, Square s) { return Square(s ^ (c * 56)); }

//        constexpr Rank relative_rank(Color c, Rank r) { return Rank(r ^ (c * 7)); }

//        constexpr Rank relative_rank(Color c, Square s) { return relative_rank(c, rank_of(s)); }

//        constexpr Direction pawn_push(Color c) { return c == WHITE ? NORTH : SOUTH; }


// Based on a congruential pseudo-random number generator
//        constexpr Key make_key(uint64_t seed) {
//            return seed * 6364136223846793005ULL + 1442695040888963407ULL;
//        }

    public static long make_key(long seed) {
        return seed * 6364136223846793005L + 1442695040888963407L;
    }


//        enum MoveType {
//            NORMAL,
//            PROMOTION  = 1 << 14,
//            EN_PASSANT = 2 << 14,
//            CASTLING   = 3 << 14
//        };

    public enum MoveType {
        NORMAL(0),
        PROMOTION(1 << 14),
        EN_PASSANT(2 << 14),
        CASTLING(3 << 14);

        public final int value;

        MoveType(int value) {
            this.value = value;
        }

        private static final MoveType[] values = values();

        public static MoveType of(int i) {
            for (MoveType mt: values) {
                if (mt.value == i) {
                    return mt;
                }
            }
            throw new IllegalArgumentException("MoveType " + i);
        }

    }

// A move needs 16 bits to be stored
//
// bit  0- 5: destination square (from 0 to 63)
// bit  6-11: origin square (from 0 to 63)
// bit 12-13: promotion piece type - 2 (from KNIGHT-2 to QUEEN-2)
// bit 14-15: special move flag: promotion (1), en passant (2), castling (3)
// NOTE: en passant bit is set only when a pawn can be captured
//
// Special cases are Move::none() and Move::null(). We can sneak these in because
// in any normal move the destination square and origin square are always different,
// but Move::none() and Move::null() have the same origin and destination square.

//        class Move {
//            public:
//            Move() = default;
//            constexpr explicit Move(std::uint16_t d) :
//            data(d) {}
//
//            constexpr Move(Square from, Square to) :
//            data((from << 6) + to) {}
//
//            template<MoveType T>
//            static constexpr Move make(Square from, Square to, PieceType pt = KNIGHT) {
//                return Move(T + ((pt - KNIGHT) << 12) + (from << 6) + to);
//            }
//
//            constexpr Square from_sq() const {
//                assert(is_ok());
//                return Square((data >> 6) & 0x3F);
//            }
//
//            constexpr Square to_sq() const {
//                assert(is_ok());
//                return Square(data & 0x3F);
//            }
//
//            constexpr int from_to() const { return data & 0xFFF; }
//
//            constexpr MoveType type_of() const { return MoveType(data & (3 << 14)); }
//
//            constexpr PieceType promotion_type() const { return PieceType(((data >> 12) & 3) + KNIGHT); }
//
//            constexpr bool is_ok() const { return none().data != data && null().data != data; }
//
//            static constexpr Move null() { return Move(65); }
//            static constexpr Move none() { return Move(0); }
//
//            constexpr bool operator==(const Move& m) const { return data == m.data; }
//            constexpr bool operator!=(const Move& m) const { return data != m.data; }
//
//            constexpr explicit operator bool() const { return data != 0; }
//
//            constexpr std::uint16_t raw() const { return data; }
//
//            struct MoveHash {
//                std::size_t operator()(const Move& m) const { return make_key(m.data); }
//            };
//
//            protected:
//            std::uint16_t data;
//        };

    /**
     * Move is short, 16-bit integer.
     * Class contains only static functions to create short etc.
     */
        public static class Move {

            public static short move(Square from, Square to) {
                return (short) ((from.ordinal() << 6) + to.ordinal());
            }

            public static short make(MoveType mt, Square from, Square to, PieceType pt) { // pt = KNIGHT
                return (short) (mt.value +
                                        ((pt.ordinal() - PieceType.KNIGHT.ordinal()) << 12) +
                                        (from.ordinal() << 6) +
                                        to.ordinal());
            }

            public static Square from_sq(short data) {
                assert(is_ok(data));
                return Square.of((data >>> 6) & 0x3F);
            }

            public static Square to_sq(short data) {
                assert(is_ok(data));
                return Square.of(data & 0x3F);
            }

            public static int from_to(short data) { return data & 0xFFF; }

            public static MoveType type_of(short data) { return MoveType.of(data & (3 << 14)); }

            public static PieceType promotion_type(short data) {
                return PieceType.of(((data >>> 12) & 3) + PieceType.KNIGHT.ordinal());
            }

            public static boolean is_ok(short data) { return none() != data && Null() != data; }

            public static short Null() { return 65; }
            public static short none() { return 0; }

//            constexpr bool operator==(const Move& m) const { return data == m.data; }
//            constexpr bool operator!=(const Move& m) const { return data != m.data; }

            public static boolean bool(short data) { return data != 0; }

//            constexpr std::uint16_t raw() const { return data; }

//            struct MoveHash {
//                std::size_t operator()(const Move& m) const { return make_key(m.data); }
//            };

//            protected short data;
        }

//    }  // namespace Stockfish

//#endif  // #ifndef TYPES_H_INCLUDED

//#include "tune.h"  // Global visibility to tuning setup

}
